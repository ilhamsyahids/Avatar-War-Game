	...
	if (PP != BuildingRelationGraphNil)
	{
		...
		if (num != 1)
		{
			...
			*AttackedBuilding = AttackedArray[AttackedBuildingIdx];
			BuildingPrintInfo(BuildingArrayElement(BuildingRecord(*G), 17));
			...
			if (success)
			{
				for(int j = 1; j <= MapMatrixLastIdxBaris(BuildingMap(*G)); j++){
					for(int k = 1; k <= MapMatrixLastIdxKolom(BuildingMap(*G)); k++){
						printf("%d ", MapMatrixElement(BuildingMap(*G), PointCreate(j,k)));				
					}
					printf("\n");
				}
				

				BuildingChangePlayer(AttackedBuilding, CurrentPlayer(*G));
				//BuildingPrintInfo(*AttackedBuilding);
